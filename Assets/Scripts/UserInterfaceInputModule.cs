...
public class UserInterfaceInputModule : PointerInputModule
{
    ...
    protected UserInterfaceInputModule()
    {
    }
    ...
    [SerializeField]
    private string m_SubmitButtonJoystick = "FaceButton A";

    [SerializeField]
    private string m_CancelButtonKeyboard = "Cancel";

    [SerializeField]
    private string m_CancelButtonJoystick = "FaceButton B";
    ...
    {
        get { return m_HorizontalAxisKeyboard; }
        set { m_HorizontalAxisKeyboard = value; }
    }
    ...
    {
        get { return m_VerticalAxisKeyboard; }
        set { m_VerticalAxisKeyboard = value; }
    }
    ...
    {
        get { return m_SubmitButtonKeyboard; }
        set { m_SubmitButtonKeyboard = value; }
    }
    ...
    {
        get { return m_CancelButtonKeyboard; }
        set { m_CancelButtonKeyboard = value; }
    }
    ...
    public override bool ShouldActivateModule()
    {
        ...
        shouldActivate |= input.GetButtonDown(m_SubmitButtonKeyboard);
        shouldActivate |= input.GetButtonDown(m_CancelButtonKeyboard);
        shouldActivate |= input.GetButtonDown(m_SubmitButtonJoystick);
        shouldActivate |= input.GetButtonDown(m_CancelButtonJoystick);
        shouldActivate |= !Mathf.Approximately(input.GetAxisRaw(m_HorizontalAxisKeyboard), 0.0f);
        shouldActivate |= !Mathf.Approximately(input.GetAxisRaw(m_VerticalAxisKeyboard), 0.0f);
        shouldActivate |= !Mathf.Approximately(input.GetAxisRaw(m_HorizontalAxisJoystick), 0.0f);
        shouldActivate |= !Mathf.Approximately(input.GetAxisRaw(m_VerticalAxisJoystick), 0.0f);
    ...
    protected bool SendSubmitEventToSelectedObject()
    {
        ...
        if (input.GetButtonDown(m_SubmitButtonKeyboard) || input.GetButtonDown(m_SubmitButtonJoystick))
            ExecuteEvents.Execute(eventSystem.currentSelectedGameObject, data, ExecuteEvents.submitHandler);

        if (input.GetButtonDown(m_CancelButtonKeyboard) || input.GetButtonDown(m_CancelButtonJoystick))
    ...
    private Vector2 GetRawMoveVector()
    {
        ...
        float xKeyboard = input.GetAxisRaw(m_HorizontalAxisKeyboard);
        float yKeyboard = input.GetAxisRaw(m_VerticalAxisKeyboard);

        float xJoystick = input.GetAxisRaw(m_HorizontalAxisJoystick);
        float yJoystick = input.GetAxisRaw(m_VerticalAxisJoystick);

        // move.x = input.GetAxisRaw(m_HorizontalAxis);
        // move.y = input.GetAxisRaw(m_VerticalAxis);

        if (input.GetButtonDown(m_HorizontalAxisKeyboard))
        {
            if (xKeyboard < 0)
                xKeyboard = -1f;
            if (xKeyboard > 0)
        ...
        if (input.GetButtonDown(m_VerticalAxisKeyboard))
        {
            if (yKeyboard < 0)
                yKeyboard = -1f;
            if (yKeyboard > 0)
        ...
        if (input.GetButtonDown(m_HorizontalAxisJoystick))
        {
            if (xJoystick < 0)
                xJoystick = -1f;
            if (xJoystick > 0)
        ...
        if (input.GetButtonDown(m_VerticalAxisJoystick))
        {
            if (yJoystick < 0)
                yJoystick = -1f;
            if (yJoystick > 0)
